...
frame_less8.sort(key=lambda l:(l[9],l[10]))
...
for one_group in frame_less8_group:
    ...
    for sn in range(len(create_sample)):
        ...
        degree_r = rdm.choice([-15,-14,-13,-12,-11,-10,-9,9,10,11,12,13,14,15]) #旋转角度
        x_list = []
        y_list = []
...
all_use_rotation=np.load(r'.\data\all_use_rotation.npy').tolist()
...
##
#all_use_rotation=np.load(r'.\data\all_use_add_rotation.npy')

# 把小于8张的都删除
all_use_rotation_pd=pd.DataFrame(all_use_rotation)
all_use_rotation_pd.columns = ['x1','y1','x2','y2','x3','y3','x4','y4','angle', 'id', 'pic','mp', 'frame','min_frame','vertical2']
frame_count=all_use_rotation_pd['pic'].drop_duplicates().groupby([all_use_rotation_pd['id'],all_use_rotation_pd['mp'],all_use_rotation_pd['min_frame'],all_use_rotation_pd['vertical2']]).size()

#找出总帧数大于等于8帧序列
frame_big8=[]
for i in range(np.shape(all_use_rotation)[0]):
    if frame_count[all_use_rotation[i,9]][all_use_rotation[i,11]][all_use_rotation[i,13]][all_use_rotation[i,14]]>=8:
        frame_big8.append(all_use_rotation[i].tolist())

np.save(r'.\data\all_use_add_rotation',frame_big8)
