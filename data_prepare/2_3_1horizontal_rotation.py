...
#因为样本中，水平的样本质量都很好，这里是为水平的样本扩充一部分质量不好的，即，随机抽一部分质量好的图片，进行旋转
...
# 角度大于9的有1415个，总43455L
##horizontal_usepd=pd.DataFrame(horizontal_use)
##horizontal_usepd.columns=['x1','y1','x2','y2','x3','y3','x4','y4','angle', 'id', 'pic','mp', 'frame','min_frame','horv']
##horizontal_use_angel_large=[]
##for i in range(np.shape(horizontal_usepd)[0]):
##    if int(horizontal_usepd['angle'].iloc[i])>=9:
##        horizontal_use_angel_large.append(1)
##    else:
##        horizontal_use_angel_large.append(0)
##horizontal_use_angel_large=pd.DataFrame(horizontal_use_angel_large)
##horizontal_use_stat=pd.concat([horizontal_usepd,horizontal_use_angel_large],axis=1)
##horizontal_use_stat.columns=['x1','y1','x2','y2','x3','y3','x4','y4','angle', 'id', 'pic','mp', 'frame','min_frame','horv','angle_large']
##horizontal_use_stat_1=horizontal_use_stat['angle_large'].groupby([horizontal_use_stat['id'],horizontal_use_stat['mp'],horizontal_use_stat['min_frame'],horizontal_use_stat['horv'],horizontal_use_stat['angle_large']]).size()
...
#    try:
#        horizontal_use_group[i].append(horizontal_use_stat_1[horizontal_use_group[i][0]][horizontal_use_group[i][1]][horizontal_use_group[i][2]][horizontal_use_group[i][3]][0])
#    except:
#        horizontal_use_group[i].append(0)
#    try:
#        horizontal_use_group[i].append(horizontal_use_stat_1[horizontal_use_group[i][0]][horizontal_use_group[i][1]][horizontal_use_group[i][2]][horizontal_use_group[i][3]][1])
#    except:
#        horizontal_use_group[i].append(0)
...
for i in range(np.shape(horizontal_use_group)[0]):
    ...
    if len(one_un_rotation_jpg)>=4:
        ...
        for sn in range(np.shape(rotation_sample_jpg)[0]):#一个帧
            image_path = r'E:\all_frame\%s' % rotation_sample_jpg[sn]
            ...
            cv2.imwrite(r'E:\frame_rotation\%s'%(this_frame[line,10]),img_rotation)
            '''
